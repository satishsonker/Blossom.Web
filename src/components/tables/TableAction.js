    ...
    return (
        <>
            <div className="table-actions d-flex align-items-center fs-6">
                ...
                {option.showView && option.view.modelId !== undefined && <div style={{ cursor: "pointer" }} onClick={e => option.view.handler(dataId, data)} className="text-primary" data-bs-placement="bottom" data-toggle="tooltip" aria-label={option.view?.title} title={option.view?.title} data-bs-toggle="modal" data-bs-target={"#" + (option.view.modelId === undefined ? "" : option.view.modelId)}><i className={option.view.icon}></i></div>}
                {option.showView && option.view.modelId === undefined && <div style={{ cursor: "pointer" }} onClick={e => option.view.handler(dataId, data)} className="text-primary" data-bs-placement="bottom" data-toggle="tooltip" aria-label={option.view?.title} title={option.view?.title}><i className={option.view.icon}></i></div>}
                ...
                {option.showDelete && <div style={{ cursor: "pointer" }}
                    data-bs-toggle={option.delete.showModel ? "modal" : ""}
                    onClick={e => !option.delete.showModel ? option.delete.handler(dataId, data, rowIndex) : () => { }}
                    data-bs-target={option.delete.showModel ? "#delete-confirm-model-" + dataId : ""}
                    className="text-danger"
                    data-bs-placement="bottom"
                    title={option.delete.title}
                    data-toggle="tooltip"
                    aria-label={option.delete?.title}><i className={option.delete.icon}></i></div>
                }
                {
                    option.buttons?.map((ele, index) => {
                        if (typeof ele.show === 'function') {
                            if (!ele.show(data, datalength)) {
                                return null;
                            }
                        }
